function classify_genotype_searchlight_para(voxels,partidx)
% searchlight, cross runs, zsocre, linear kernel libsvm
% 2013-08-05
% fsl_sub -l ./log/ matlab -nodesktop -nosplash -r "classify_genotype_searchlight_para(1:5,1);quit;"
...
para=1000;
voxelaccuracy=zeros(1,length(voxels)); % check if nSl is number of voxels
tic;
    ...
    for iSl= 1:length(voxels) % searchlight for each voxel
        iSl
        temp=ix123(ixMask(voxels(iSl)),sz);
...
save(sprintf('results/classify_genotype_%d.mat',partidx),'voxelaccuracy');
